...
<body>
	...
	<div id="main">
		...
		<c:forEach var="message" items="${userMessagesOut }">
			 <tr>
		 		...
		 		<form action=MessageInfo method=get>
		 			<c:choose>
		 				<c:when test="${message.status==0 }">
		 					<input type="hidden" name ="message" value="${message.helper }">
		 					<input type="hidden" name ="outIn" value="Outbox" >
		 					<input style="color:red" type="submit" value="${message.subject}"> 
		 				</c:when>
		 				<c:otherwise>
		 					<input type="hidden" name ="message" value="${message.helper }">
		 					<input type="hidden" name ="outIn" value="Outbox" >
		 					<input type="submit" value="${message.subject}"> 
		 				</c:otherwise>
		 			</c:choose>
		 		</form>
		 		 </td>
